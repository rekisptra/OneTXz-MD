...
function sholatAll(kota) {
    return new Promise(async (resolve, reject) => {
        ...
        }); resolve({ creator: '@OneTXz', status: true, data })
    })
}
...
module.exports = (kota) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (!sholat.status) return resolve({ creator: '@onetxz', status: false })
        let now = moment(new Date * 1).format('DD-MM-yy')
        let data = sholat.data.find(v => v.tanggal == now)
        if (typeof data == 'undefined') return resolve({ creator: '@OneTXz', status: false })
        resolve({ creator: '@OneTXz', status: true, data })
    })
}